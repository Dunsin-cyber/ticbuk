...
func NewAuthService(authRepo models.AuthRepository) models.AuthService {
	return &AuthService{
		authRepo: authRepo,
	}
}
...
func (s *AuthService) Register(ctx context.Context, registerData *models.AuthCredentials) (string, *models.User, error) {
	...
	user, err = s.authRepo.RegisterUser(ctx, registerData)
	if err != nil {
		return "", nil, err
	}
...
func (s *AuthService) Login(ctx context.Context, loginData *models.AuthCredentials) (string, *models.User, error) {
	if !models.IsValidEmail(loginData.Email) {
		return "", nil, fmt.Errorf("invalid email format")
	}

	user, err := s.authRepo.GetUser(ctx, "email = ?", loginData.Email)

	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return "", nil, fmt.Errorf("invlaid credentials")
		}
		return "", nil, err
	}
